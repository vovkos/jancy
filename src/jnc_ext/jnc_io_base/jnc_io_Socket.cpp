...
JNC_BEGIN_TYPE_FUNCTION_MAP(Socket)
	...
	JNC_MAP_PROPERTY("m_tcpKeepAliveIdleTimeout", &Socket::getTcpKeepAliveIdleTimeout, &Socket::setTcpKeepAliveIdleTimeout)
	JNC_MAP_PROPERTY("m_tcpKeepAliveRetryInterval", &Socket::getTcpKeepAliveRetryInterval, &Socket::setTcpKeepAliveRetryInterval)
...
Socket::sendRecvLoop(
	...
	for (;;) {
		...
		if (!isSendingSocket)
			if (isDatagram) {
				if (!m_overlappedIo->m_sendToParams.isEmpty()) {
					ASSERT(m_overlappedIo->m_sendToParams.getCount() >= sizeof(axl::io::SockAddr));

					result = m_socket.m_socket.wsaSendTo(
			...
			} else if (!m_overlappedIo->m_sendBlock.isEmpty()) {
	...
	for (;;) {
		...
		if (isDatagram) {
			...
			while (canWriteSocket) {
				...
				ASSERT(writeParams.getCount() >= sizeof(axl::io::SockAddr));
