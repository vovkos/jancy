...
NamespaceMgr::addStdItems() {
	...
	bool result =
		...
		jncNspace->addItem("global", globalNspace) && // analogue of :: in C++
		...
		jncNspace->addItem("limitDataPtr", dataPtrImport) &&
		jncNspace->addItem("getDataPtrLeftRadius", dataPtrImport) &&
		jncNspace->addItem("getDataPtrRightRadius", dataPtrImport) &&
...
NamespaceMgr::openNamespace(Namespace* nspace) {
	...
	m_currentScope = NULL; // we (unfortunately) often rely on scope being NULL or not to make decisions
